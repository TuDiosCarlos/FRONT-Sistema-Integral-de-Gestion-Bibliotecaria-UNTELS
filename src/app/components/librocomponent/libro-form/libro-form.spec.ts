...
describe('LibroForm', () => {
  let libroServiceSpy: {
    ...
    autocompletarPorIsbn: ReturnType<typeof vi.fn>;
  };
  ...
  function crearComponente(
    ...
    libroServiceSpy = {
      ...
      autocompletarPorIsbn: vi.fn().mockReturnValue(of({} as LibroApiExterna)),
    };
  ...
  it('buscarPorIsbn() rellena los datos bibliográficos sin guardar ni navegar', () => {
    const { componentInstance: component } = crearComponente();
    libroServiceSpy.autocompletarPorIsbn.mockReturnValue(of({
      isbn: '9780134685991',
      titulo: 'Effective Java',
      autor: 'Joshua Bloch',
      editorial: 'Addison-Wesley',
      anio: 2018,
      descripcion: 'Buenas prácticas de Java',
    } as LibroApiExterna));

    component.form.patchValue({ isbn: '9780134685991' });
    component.buscarPorIsbn();

    expect(libroServiceSpy.autocompletarPorIsbn).toHaveBeenCalledWith('9780134685991');
    expect(component.form.get('titulo')?.value).toBe('Effective Java');
    expect(component.form.get('autor')?.value).toBe('Joshua Bloch');
    expect(component.cargandoIsbn).toBe(false);
    expect(libroServiceSpy.registrar).not.toHaveBeenCalled();
    expect(routerNavigateSpy).not.toHaveBeenCalled();
  });

  it('buscarPorIsbn() no rellena el formulario si el ISBN ya existe en el catálogo (409)', () => {
    const { componentInstance: component } = crearComponente();
    libroServiceSpy.autocompletarPorIsbn.mockReturnValue(throwError(() => ({ status: 409 })));

    component.form.patchValue({ isbn: '9780132350884', titulo: '' });
    component.buscarPorIsbn();
